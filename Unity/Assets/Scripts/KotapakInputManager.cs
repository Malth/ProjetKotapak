...
public class KotapakInputManager : MonoBehaviour {
	...
	/// </param>
	[RPC]
	void CheckSurvivorAndDie(string tagPlayer)
	{
				int indicePlayer = 0;
				switch (tagPlayer) {
				case"player1":
				...
				isDead [indicePlayer] = true;

		if(int.Parse (Network.player.ToString())- 1 == indicePlayer)
		...
		if(i==1)Application.LoadLevel("winner");


		if (Network.isServer)
		{
			_myNetworkView.RPC("CheckSurvivorAndDie", RPCMode.OthersBuffered, tagPlayer );
		}
	}
	/// <summary>
